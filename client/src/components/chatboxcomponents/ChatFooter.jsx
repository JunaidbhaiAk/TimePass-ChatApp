...
const ChatFooter = ({settypedmessage,sendmsg,v}) => {
    ...
    <Box backgroundColor="#9fb1bcff" width="100%" height="41px" padding="5px" borderBottomRightRadius="10px">
      <InputGroup>
        <InputRightElement
          ...
          onClick={sendmsg}
        />
        {activeUsers?.some(ele=> ele.id === person._id) ? <Input type="text" placeholder="Enter Message" size="sm" backgroundColor="#fff" borderRadius="20px" onChange={(e)=>{settypedmessage(e.target.value)}} value={v}/> : <Input type="text" placeholder="Sorry Cant Message When User is Offline" size="sm" backgroundColor="#fff" borderRadius="20px" disabled _placeholder={{color:"#FF0000"}}/>}
      </InputGroup>
    </Box>
  );
};
